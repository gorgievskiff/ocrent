...
exports.handleValueAndCheck = exports.getInputValue = void 0;
const getInputValue = (el, clockType, currentTime, updateOptions) => {
    if (!el) {
        return {
            hour: '12',
            minutes: '00',
    ...
    if (!currentTime) {
        ...
        if (preventClockType && updateOptions) {
            const [preventHour, splitedRest] = new Date(cTime).toLocaleTimeString().split(':');
        ...
        const [hour, splitMinutes] = new Date(cTime)
    ...
    if (clockType === '12h') {
        if (hor > 12 || min > 59 || min < 0 || hor === 0 || (type !== 'AM' && type !== 'PM')) {
            return {
                error: `The input contains invalid letters or numbers. Problem is with hour which should be less than 13 and higher or equal 0, currentHour: ${hor}. Minutes should be less than 60 and higher or equal 0, currentMinutes: ${Number(min)} or invalid type (PM or AM), currentType: ${type}.`,
                currentHour: hor,
                currentMin: min,
                currentType: type,
            };
        ...
        if (hor < 0 || hor > 23 || min > 59) {
            return {
                error: `The input contains invalid numbers. Problem is with hour which should be less than 24 and higher or equal 0, currentHour: ${hor}. Minutes should be less than 60 and higher or equal 0, currentMinutes: ${Number(min)}`,
                currentHour: hor,
                currentMin: min,
            };
        }
...
const handleValueAndCheck = (val, type, clockType) => {
...
exports.handleValueAndCheck = handleValueAndCheck;
